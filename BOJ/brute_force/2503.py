...
# 영수가 324를 갖고 있으면 
# 429는 1 스트라이크 1 볼이다. # 241은 0 스트라이크 2 볼이다. # 924는 2 스트라이크 0 볼이다.
...
for _ in range(N):
    ...
    for idx in range (len(numbers)):
        s, b = 0, 0
        idx -= remove_cnt # 중요! 정해진 len(numbers)만큼 돌아가긴 하지만, idx는 numbers의 [0]부터 시작하도록.
        for j in range (3): 
            if num[j] == numbers[idx][j]:
                s += 1
            elif num[j] in numbers[idx]:
                b += 1
...
# 진심 for문에 저 remove_cnt 진짜 너무 열받아 아오